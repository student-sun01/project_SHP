  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      >
        ...
        >
        <p @click="changDefault(userAddress, userAddressList)">
          <span class="s1">{{ userAddress.userAddress }}</span>
          <span class="s2">{{ userAddress.userAddress }}</span>
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{ defaultChangeAdress.userAddress }}</span>
        收货人：<span>{{ defaultChangeAdress.consignee }}</span>
        <span>{{ defaultChangeAdress.phoneNum }}</span>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    // 切换默认地址  排他
    changDefault(userAddress, userAddressList) {
      userAddressList.forEach((item) => (item.isDefault = "0"));
      userAddress.isDefault = "1";
    },
    //提交订单
    async submitOrder() {
      // 带上交易编号和交易信息发送请求创建订单获取订单编号
      let tradeNo = this.tradeInfo.tradeNo;
      let tradeData = {
        consignee: this.defaultChangeAdress.consignee,
        consigneeTel: this.defaultChangeAdress.phoneNum,
        deliveryAddress: this.defaultChangeAdress.userAddress,
        paymentWay: "ONLINE",
